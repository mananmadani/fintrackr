...
function createPieChart(canvasId, labels, data, colorArray) {
  ...
  const chart = new Chart(canvas, {
    type: 'pie',
    data: { labels: labels, datasets: [{ data: data, backgroundColor: colorArray }] },
    options: {
      responsive: false,
...
function getColorArray(n) {
  const base = ['#00BFA6', '#1E88E5', '#E53935', '#FFC107', '#43A047', '#8E24AA', '#6D4C41', '#0288d1', '#cddc39', '#ec407a', '#ff9800', '#4caf50', '#bdbdbd', '#6d4c41', '#e91e63', '#90caf9', '#388e3c', '#ffb300'];
  return Array(n).fill().map((_, i) => base[i % base.length]);
}
function uniqueCategories(arr) {
  return [...new Set(arr.map(e => (e.category || '').trim()).filter(x => !!x))];
}
function updateAutocompletes() {
  const data = getData();
  const incList = document.getElementById('inc-category-list');
  incList.innerHTML = '';
  uniqueCategories(data.income).forEach(cat => incList.innerHTML += `<option value="${cat}">`);
  const expList = document.getElementById('exp-category-list');
  expList.innerHTML = '';
  uniqueCategories(data.expenses).forEach(cat => expList.innerHTML += `<option value="${cat}">`);
}
...
document.getElementById('export-pdf-btn').addEventListener('click', function() {
  ...
  if (incomeCanvas.chartInstance && incomeCanvas.chartInstance.data.labels.length > 0) {
    doc.addImage(incomeCanvas.chartInstance.toBase64Image(), 'PNG', 10, y, 60, 60);
  }
  if (expenseCanvas.chartInstance && expenseCanvas.chartInstance.data.labels.length > 0) {
    doc.addImage(expenseCanvas.chartInstance.toBase64Image(), 'PNG', 75, y, 60, 60);
  }
