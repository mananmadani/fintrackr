...
function drawOrMessage(chartElement, labels, message) {
  let wrap = chartElement.closest('.chart-wrap');
...
function updatePieCharts() {
  ...
  // Chart.js instance for income
  let incomePie = document.getElementById('income-pie');
  incomeChart = new Chart(incomePie, {
    type: 'pie',
    data: { labels: incCats, datasets: [{ data: incVals, backgroundColor: getColorArray(incCats.length) }] },
    options: { plugins: { legend: { position: 'bottom' } }, responsive:false, maintainAspectRatio: false }
  });
  drawOrMessage(incomePie, incCats, 'No income data');
  // Chart.js instance for expense
  let expensePie = document.getElementById('expense-pie');
  expenseChart = new Chart(expensePie, {
    type: 'pie',
    data: { labels: expCats, datasets: [{ data: expVals, backgroundColor: getColorArray(expCats.length) }] },
    options: { plugins: { legend: { position: 'bottom' } }, responsive:false, maintainAspectRatio: false }
  });
  drawOrMessage(expensePie, expCats, 'No expense data');
}
...
document.getElementById('export-pdf-btn').addEventListener('click', function() {
  ...
  if (incomeChart && incomeChart.data.labels.length > 0) { doc.addImage(incomeChart.toBase64Image(), 'PNG', 10, y, 40, 40); }
  if (expenseChart && expenseChart.data.labels.length > 0) { doc.addImage(expenseChart.toBase64Image(), 'PNG', 55, y, 40, 40); }
